...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openwolf/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenWolf Re-Auth" -c "After getting key, run: ssh $SERVER '~/openwolf/scripts/mobile-reauth.sh'" --id openwolf-auth
        ;;
    *)
