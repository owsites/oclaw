...
#
# Suggested cron: */30 * * * * /home/admin/openwolf/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenWolf notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openwolf-alerts)
...
STATE_FILE="$HOME/.openwolf/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenWolf if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openwolf
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenWolf to $NOTIFY_PHONE..."
            openwolf send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenWolf is down. Run: ssh l36 '~/openwolf/scripts/mobile-reauth.sh'" "urgent"
    exit 1
