...
const shouldBuild = (deps) => {
  if (deps.env.OPENWOLF_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENWOLF_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openwolf] ${message}\n`);
};

const runOpenWolf = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openwolf.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
