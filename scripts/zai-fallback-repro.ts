...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "openwolf.json");
  ...
  const sessionId =
    process.env.OPENWOLF_ZAI_FALLBACK_SESSION_ID ??
    process.env.WOLFBOT_ZAI_FALLBACK_SESSION_ID ??
    randomUUID();
  ...
  const baseEnv: NodeJS.ProcessEnv = {
    ...
    WOLFBOT_CONFIG_PATH: configPath,
    WOLFBOT_STATE_DIR: stateDir,
    ZAI_API_KEY: zaiKey,
    Z_AI_API_KEY: "",
  };
  ...
  const run1 = await runCommand(
    "run1",
    ["openwolf", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
    envValidAnthropic,
  );
  if (run1.code !== 0) {
  ...
  const run2 = await runCommand(
    "run2",
    ["openwolf", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
    envInvalidAnthropic,
  );
