...
BASE_IMAGE="${BASE_IMAGE:-openwolf-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openwolf-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openwolf.sandbox=1)
NOTE
