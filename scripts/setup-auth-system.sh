#!/bin/bash
# Setup OpenWolf Auth Management System
...
echo "=== OpenWolf Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openwolf-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenWolf message: Send warning via OpenWolf itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openwolf-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openwolf-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openwolf-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openwolf-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openwolf-auth-monitor.timer"
echo ""
echo "Quick commands:"
