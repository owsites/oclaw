...
IMAGE_NAME="openwolf-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openwolf-net-e2e-$$"
GW_NAME="openwolf-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "OPENWOLF_GATEWAY_TOKEN=$TOKEN" \
	  -e "OPENWOLF_SKIP_CHANNELS=1" \
	  -e "OPENWOLF_SKIP_GMAIL_WATCHER=1" \
	  -e "OPENWOLF_SKIP_CRON=1" \
	  -e "OPENWOLF_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
