...
IMAGE_NAME="openwolf-plugins-e2e"
...
echo "Running plugins Docker E2E..."
	docker run --rm -t "$IMAGE_NAME" bash -lc '
	  ...
	  if [ -f dist/index.mjs ]; then
	    OPENWOLF_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    OPENWOLF_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export OPENWOLF_ENTRY

	  home_dir=$(mktemp -d "/tmp/openwolf-plugins-e2e.XXXXXX")
	  export HOME="$home_dir"
  mkdir -p "$HOME/.openwolf/extensions/demo-plugin"

  cat > "$HOME/.openwolf/extensions/demo-plugin/index.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
  cat > "$HOME/.openwolf/extensions/demo-plugin/openwolf.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$OPENWOLF_ENTRY" plugins list --json > /tmp/plugins.json
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/openwolf-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@openwolf/demo-plugin-tgz",
  "version": "0.0.1",
...
JS
  cat > "$pack_dir/package/openwolf.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
JSON
  tar -czf /tmp/demo-plugin-tgz.tgz -C "$pack_dir" package
	  ...
	  node "$OPENWOLF_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
	  node "$OPENWOLF_ENTRY" plugins list --json > /tmp/plugins2.json
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/openwolf-plugin-dir.XXXXXX")"
  cat > "$dir_plugin/package.json" <<'"'"'JSON'"'"'
{
  "name": "@openwolf/demo-plugin-dir",
  "version": "0.0.1",
  "openwolf": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$dir_plugin/openwolf.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$OPENWOLF_ENTRY" plugins install "$dir_plugin"
	  node "$OPENWOLF_ENTRY" plugins list --json > /tmp/plugins3.json
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/openwolf-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@openwolf/demo-plugin-file",
  "version": "0.0.1",
  "openwolf": { "extensions": ["./index.js"] }
}
JSON
...
JS
  cat > "$file_pack_dir/package/openwolf.plugin.json" <<'"'"'JSON'"'"'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
JSON
	  ...
	  node "$OPENWOLF_ENTRY" plugins install "file:$file_pack_dir/package"
	  node "$OPENWOLF_ENTRY" plugins list --json > /tmp/plugins4.json
