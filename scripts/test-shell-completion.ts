 ...
 * `openwolf update`. Use it to verify the completion installation flow
...
\`openwolf update\`, \`openwolf doctor\`, and \`openwolf onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenWolf Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openwolf completion --install\` later to enable.`));
    return;
  }
