...
import type { ApiContributor, Entry, MapConfig, User } from "./update-wolftributors.types.js";

const REPO = "openwolf/openwolf";
const PER_LINE = 10;

const mapPath = resolve("scripts/wolftributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing wolftributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README wolftributors: ${entries.length} entries`);
