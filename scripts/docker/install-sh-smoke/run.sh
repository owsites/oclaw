...
INSTALL_URL="${OPENWOLF_INSTALL_URL:-https://openwolf.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENWOLF_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENWOLF_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openwolf"
PACKAGE_NAME="${OPENWOLF_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENWOLF_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENWOLF_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENWOLF_INSTALL_LATEST_OUT:-}"
fi
