...
# Build and bundle OpenWolf into a minimal .app we can open.
# Outputs to dist/OpenWolf.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenWolf.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenWolf"
BUNDLE_ID="${BUNDLE_ID:-ai.openwolf.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openwolf/openwolf/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenWolf/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenWolfBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenWolfGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenWolf"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenWolf"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenWolf"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenWolf" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenWolf/Resources/OpenWolf.icns" "$APP_ROOT/Contents/Resources/OpenWolf.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenWolf/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenWolfKit resources"
OPENWOLFKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenWolfKit_OpenWolfKit.bundle"
if [ -d "$OPENWOLFKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenWolfKit_OpenWolfKit.bundle"
  cp -R "$OPENWOLFKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenWolfKit_OpenWolfKit.bundle"
else
  echo "WARN: OpenWolfKit resource bundle not found at $OPENWOLFKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenWolf"
killall -q OpenWolf 2>/dev/null || true
