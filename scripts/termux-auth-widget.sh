...
# OpenWolf Auth Widget for Termux
...
SERVER="${OPENWOLF_SERVER:-${WOLFBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking OpenWolf auth..."

STATUS=$(ssh "$SERVER" '$HOME/openwolf/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/openwolf/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENWOLF_EXPIRING|WOLFBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/openwolf/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENWOLF_EXPIRED|WOLFBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenWolf won't work until re-auth"
                ;;
        esac
        ;;
