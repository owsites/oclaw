...
OpenWolf integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenWolf owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenWolf spawns `imsg rpc` as a child process (legacy iMessage integration).
