...
OpenWolf features that can generate provider usage or paid API calls.
...
- `openwolf status --usage` and `openwolf channels list` show provider **usage windows**
...
OpenWolf can pick up credentials from:
...
- `openwolf status --usage`
- `openwolf models status --json`
...
`openwolf models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
