...
This document explains how OpenWolf manages sessions end-to-end:
...
OpenWolf is designed around a single **Gateway process** that owns session state.
...
OpenWolf persists sessions in two layers:
...
- Transcripts: `~/.openwolf/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

OpenWolf resolves these via `src/config/sessions.ts`.
...
OpenWolf intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (OpenWolf consumes the events, but Pi decides when to compact).
...
OpenWolf also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, OpenWolf bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, OpenWolf leaves it alone.
...
- `openwolf status` (CLI)
- `openwolf sessions` / `sessions --json`
...
OpenWolf supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- OpenWolf strips/suppresses this in the delivery layer.

As of `2026.1.10`, OpenWolf also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
OpenWolf uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but OpenWolf’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `openwolf status`.
