---
summary: "OpenWolf on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenWolf on a Raspberry Pi
  - Running OpenWolf on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenWolf on Raspberry Pi

## Goal

Run a persistent, always-on OpenWolf Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenWolf features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openwolf

# Enable if not
sudo systemctl enable openwolf

# Start on boot
sudo systemctl start openwolf
```
...
```bash
# Check logs
journalctl -u openwolf --no-pager -n 100

# Common fix: rebuild
cd ~/openwolf  # if using hackable install
npm run build
sudo systemctl restart openwolf
```
