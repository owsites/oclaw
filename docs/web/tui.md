...
```bash
openwolf tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openwolf tui --deliver`
...
- Check the Gateway logs: `openwolf logs --follow`.
- Confirm the agent can run: `openwolf status` and `openwolf models status`.
...
- No agents in picker: check `openwolf agents list` and your routing config.
