...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openwolf`)
...
If the page fails to load, start the Gateway first: `openwolf gateway`.
...
you revoke it with `openwolf devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openwolf/openwolf.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenWolf
...
OpenWolf **blocks** Control UI connections without device identity.
