...
- Jobs persist under `~/.openwolf/cron/` so restarts don’t lose schedules.
...
openwolf cron list
openwolf cron run <job-id>
openwolf cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openwolf/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openwolf cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, OpenWolf defaults to `announce`.
...
- Job store: `~/.openwolf/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openwolf/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
openwolf cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openwolf cron edit <jobId> --agent ops
openwolf cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
openwolf cron run <jobId>
openwolf cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
openwolf cron edit <jobId> \
...
```bash
openwolf system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openwolf system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENWOLF_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenWolf applies exponential retry backoff for recurring jobs after consecutive errors:
