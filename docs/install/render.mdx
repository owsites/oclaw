...
Deploy OpenWolf on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/openwolf/openwolf)
...
repository configures everything needed to run OpenWolf:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENWOLF_STATE_DIR
        value: /data/.openwolf
      - key: OPENWOLF_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENWOLF_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/openwolf`.
...
If you use the original OpenWolf repository, Render will not auto-deploy your OpenWolf. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenWolf, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenWolf host.
