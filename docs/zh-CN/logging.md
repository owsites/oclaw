...
你可以在 `~/.openwolf/openwolf.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/openwolf.log"
  }
}
```
...
```bash
openwolf channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.openwolf/openwolf.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/openwolf/openwolf-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- OpenWolf 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
OPENWOLF_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `openwolf plugins enable diagnostics-otel` 启用插件。
...
- `openwolf.tokens`（计数器，属性：`openwolf.token`、`openwolf.channel`、`openwolf.provider`、`openwolf.model`）
- `openwolf.cost.usd`（计数器，属性：`openwolf.channel`、`openwolf.provider`、`openwolf.model`）
- `openwolf.run.duration_ms`（直方图，属性：`openwolf.channel`、`openwolf.provider`、`openwolf.model`）
- `openwolf.context.tokens`（直方图，属性：`openwolf.context`、`openwolf.channel`、`openwolf.provider`、`openwolf.model`）

消息流：

- `openwolf.webhook.received`（计数器，属性：`openwolf.channel`、`openwolf.webhook`）
- `openwolf.webhook.error`（计数器，属性：`openwolf.channel`、`openwolf.webhook`）
- `openwolf.webhook.duration_ms`（直方图，属性：`openwolf.channel`、`openwolf.webhook`）
- `openwolf.message.queued`（计数器，属性：`openwolf.channel`、`openwolf.source`）
- `openwolf.message.processed`（计数器，属性：`openwolf.channel`、`openwolf.outcome`）
- `openwolf.message.duration_ms`（直方图，属性：`openwolf.channel`、`openwolf.outcome`）

队列 + 会话：

- `openwolf.queue.lane.enqueue`（计数器，属性：`openwolf.lane`）
- `openwolf.queue.lane.dequeue`（计数器，属性：`openwolf.lane`）
- `openwolf.queue.depth`（直方图，属性：`openwolf.lane` 或 `openwolf.channel=heartbeat`）
- `openwolf.queue.wait_ms`（直方图，属性：`openwolf.lane`）
- `openwolf.session.state`（计数器，属性：`openwolf.state`、`openwolf.reason`）
- `openwolf.session.stuck`（计数器，属性：`openwolf.state`）
- `openwolf.session.stuck_age_ms`（直方图，属性：`openwolf.state`）
- `openwolf.run.attempt`（计数器，属性：`openwolf.attempt`）

### 导出的 span（名称 + 关键属性）

- `openwolf.model.usage`
  - `openwolf.channel`、`openwolf.provider`、`openwolf.model`
  - `openwolf.sessionKey`、`openwolf.sessionId`
  - `openwolf.tokens.*`（input/output/cache_read/cache_write/total）
- `openwolf.webhook.processed`
  - `openwolf.channel`、`openwolf.webhook`、`openwolf.chatId`
- `openwolf.webhook.error`
  - `openwolf.channel`、`openwolf.webhook`、`openwolf.chatId`、`openwolf.error`
- `openwolf.message.processed`
  - `openwolf.channel`、`openwolf.outcome`、`openwolf.chatId`、`openwolf.messageId`、`openwolf.sessionKey`、`openwolf.sessionId`、`openwolf.reason`
- `openwolf.session.stuck`
  - `openwolf.state`、`openwolf.ageMs`、`openwolf.queueDepth`、`openwolf.sessionKey`、`openwolf.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openwolf doctor`。
