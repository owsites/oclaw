...
description: "Complete field-by-field reference for ~/.openwolf/openwolf.json"
---

# Configuration Reference

Every field available in `~/.openwolf/openwolf.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenWolf uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `openwolf doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["openwolf-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `openwolf plugins install @openwolf/mattermost`.
...
OpenWolf spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openwolf"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openwolf.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenWolf auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenWolf assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.openwolf/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.openwolf/workspace",
        agentDir: "~/.openwolf/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@openwolf"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openwolf`   | All built-in tools (excludes provider plugins)                                           |
...
OpenWolf uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openwolf/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENWOLF_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openwolf onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openwolf onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openwolf onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openwolf onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openwolf onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openwolf onboard --auth-choice minimax-api`.
...
- Loaded from `~/.openwolf/extensions`, `<workspace>/.openwolf/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENWOLF_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENWOLF_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.openwolf-dev` + port `19001`), `--profile <name>` (uses `~/.openwolf-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.openwolf/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-openwolf-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENWOLF_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENWOLF_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__openwolf__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openwolf__/a2ui/`
...
- Also serves A2UI at `/__openwolf__/a2ui/`.
...
- Hostname defaults to `openwolf`. Override with `OPENWOLF_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.openwolf/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `openwolf dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.openwolf/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPENWOLF_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.openwolf/credentials/oauth.json`.
...
- Default log file: `/tmp/openwolf/openwolf-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openwolf doctor --fix` can strip unknown keys).
