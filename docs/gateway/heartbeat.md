...
- During heartbeat runs, OpenWolf treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openwolf sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenWolf skips the heartbeat run entirely (no model call).
...
```bash
openwolf system event --text "Check for urgent follow-ups" --mode now
```
