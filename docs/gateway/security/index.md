...
## Quick check: `openwolf security audit`
...
```bash
openwolf security audit
openwolf security audit --deep
openwolf security audit --fix
```
...
- Tighten local perms (`~/.openwolf` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

OpenWolf is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, OpenWolf also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.openwolf/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.openwolf/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.openwolf/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.openwolf/credentials/oauth.json`
...
`openwolf security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENWOLF_GATEWAY_PASSWORD}
```
...
OpenWolf stores session transcripts on disk under `~/.openwolf/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.openwolf` (see the audit section below). If you need
...
OpenWolf can refresh the skills list mid-session:
...
- If you install plugins from npm (`openwolf plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openwolf/extensions/<pluginId>/` (or `$OPENWOLF_STATE_DIR/extensions/<pluginId>/`).
  - OpenWolf uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
openwolf pairing list <channel>
openwolf pairing approve <channel> <code>
```
...
By default, OpenWolf routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenWolf has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.openwolf/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.openwolf or your logs.”
...
4. **Re-run audit**
   - `openwolf security audit --deep` and confirm the report is clean.
...
- `~/.openwolf/openwolf.json`: `600` (user read/write only)
- `~/.openwolf`: `700` (user only)

`openwolf doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENWOLF_GATEWAY_PORT`

This HTTP surface includes the Control UI and the canvas host:

- Control UI (SPA assets) (default base path `/`)
- Canvas host: `/__openwolf__/canvas/` and `/__openwolf__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
The Gateway broadcasts its presence via mDNS (`_openwolf-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENWOLF_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openwolf doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENWOLF_GATEWAY_PASSWORD`).
...
1. Generate/set a new secret (`gateway.auth.token` or `OPENWOLF_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenWolf
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. OpenWolf verifies the identity by resolving the
...
- OpenWolf will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openwolf/` (or `$OPENWOLF_STATE_DIR/`) may contain secrets or private data:

- `openwolf.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openwolf status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openwolf", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openwolf/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openwolf` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenWolf clients can connect.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default OpenWolf limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openwolf gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENWOLF_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/openwolf/openwolf-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openwolf/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + OpenWolf version
...
Found a vulnerability in OpenWolf? Please report responsibly:

1. Email: [security@openwolf.ai](mailto:security@openwolf.ai)
