...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenWolf
...
Running OpenWolf via Node with `tsx` fails at startup with:

```
[openwolf] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openwolf status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openwolf.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openwolf.mjs status` works on Node 25.
