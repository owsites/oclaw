...
read_when:
  - You need detailed behavior for openwolf onboard
...
This page is the full reference for `openwolf onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.openwolf/openwolf.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `openwolf doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `openwolf pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `openwolf health`.
    - `openwolf status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.openwolf/.env` so launchd can read it.
...
- OAuth credentials: `~/.openwolf/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.openwolf/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.openwolf/credentials/oauth.json` (or `$OPENWOLF_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`openwolf onboard`](/cli/onboard)
