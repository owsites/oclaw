...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openwolf sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openwolf doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openwolf-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENWOLF_STATE_DIR:-$HOME/.openwolf}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
