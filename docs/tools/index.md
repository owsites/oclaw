---
summary: "Agent tool surface for OpenWolf (browser, canvas, nodes, message, cron) replacing legacy `openwolf-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openwolf-*` skills
title: "Tools"
---

# Tools (OpenWolf)

OpenWolf exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openwolf-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openwolf.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenWolf logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:openwolf`: all built-in OpenWolf tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`openwolf node run`).
...
- Requires a Brave API key (recommended: `openwolf configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenWolf-managed browser.
...
- Quick smoke: `openwolf nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openwolf gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, OpenWolf clamps `tools.sessions.visibility` to `tree`.
