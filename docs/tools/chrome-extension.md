---
summary: "Chrome extension: let OpenWolf drive your existing Chrome tab"
read_when:
...
The OpenWolf Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openwolf-managed Chrome profile.
...
OpenWolf then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenWolf release (npm package) as static files. There is no separate “build” step.

After upgrading OpenWolf:

- Re-run `openwolf browser extension install` to refresh the installed files under your OpenWolf state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenWolf ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenWolf to control.
...
- `ON`: attached; OpenWolf can drive that tab.
...
Debugging: `openwolf sandbox explain`
...
`openwolf browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openwolf browser extension install` first to copy the extension to a stable location under your OpenWolf state directory.
...
- **This is not isolated** like the dedicated openwolf-managed profile.
