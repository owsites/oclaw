...
- `/usage cost` in chats: local cost summary aggregated from OpenWolf session logs.
- CLI: `openwolf status --usage` prints a full per-provider breakdown.
- CLI: `openwolf channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
