...
read_when:
  - You want OpenWolf to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenWolf to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`openwolf onboard`) and `openwolf channels add` list optional channel plugins.
...
```bash
openwolf plugins install @openwolf/nostr
```

Use a local checkout (dev workflows):

```bash
openwolf plugins install --link <path-to-openwolf>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "openwolf@example.com",
        "lud16": "openwolf@example.com"
      }
    }
  }
}
