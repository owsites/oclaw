...
Tlon is a decentralized messenger built on Urbit. OpenWolf connects to your Urbit ship and can
...
```bash
openwolf plugins install @openwolf/tlon
```

Local checkout (when running from a git repo):

```bash
openwolf plugins install ./extensions/tlon
```
...
By default, OpenWolf blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `openwolf message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenWolf replies in-thread.
