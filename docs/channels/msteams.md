...
```bash
openwolf plugins install @openwolf/msteams
```

Local checkout (when running from a git repo):

```bash
openwolf plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OpenWolf will offer the local install path automatically.
...
3. Configure OpenWolf with those credentials.
...
- Talk to OpenWolf via Teams DMs, group chats, or channels.
...
- On startup, OpenWolf resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.openwolf/openwolf.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OpenWolf, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `openwolf-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `openwolf plugins install @openwolf/msteams`
   - From a local checkout: `openwolf plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OpenWolf in Teams", "full": "OpenWolf in Teams" },
...
OpenWolf handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OpenWolf only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpenWolfShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OpenWolf sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openwolf message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.openwolf/msteams-polls.json`.
...
```bash
openwolf message send --channel msteams \
...
openwolf message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
openwolf message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
openwolf message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
openwolf message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
