...
```bash
openwolf plugins install @openwolf/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openwolf plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenWolf" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openwolf pairing list nextcloud-talk`
  - `openwolf pairing approve nextcloud-talk <CODE>`
