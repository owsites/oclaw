...
Matrix is an open, decentralized messaging protocol. OpenWolf connects as a Matrix **user**
...
```bash
openwolf plugins install @openwolf/matrix
```

Local checkout (when running from a git repo):

```bash
openwolf plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openwolf plugins install @openwolf/matrix`
   - From a local checkout: `openwolf plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenWolf calls the same
     login endpoint, stores the access token in `~/.openwolf/credentials/matrix/credentials.json`,
...
- On first connection, OpenWolf requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenWolf logs a warning.
...
`~/.openwolf/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openwolf pairing list matrix`
  - `openwolf pairing approve matrix <CODE>`
...
- On startup, OpenWolf resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
