...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenWolf to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
openwolf plugins install @openwolf/feishu
```

Local checkout (when running from a git repo):

```bash
openwolf plugins install ./extensions/feishu
```
...
If you just installed OpenWolf, run the wizard:
...
2. Configuring app credentials in OpenWolf
...
1. You already ran `openwolf channels add` for Feishu
2. The gateway is running (`openwolf gateway status`)
...
```bash
openwolf pairing approve feishu <CODE>
```
...
2. Run `openwolf logs --follow` and look for `chat_id`
...
2. Run `openwolf logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openwolf gateway status`  | Show gateway status           |
| `openwolf gateway install` | Install/start gateway service |
| `openwolf gateway stop`    | Stop gateway service          |
| `openwolf gateway restart` | Restart gateway service       |
| `openwolf logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openwolf logs --follow`
...
5. Ensure the gateway is running: `openwolf gateway status`
6. Check logs: `openwolf logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.openwolf/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.openwolf/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
