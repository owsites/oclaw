...
read_when:
  - Setting up Twitch chat integration for OpenWolf
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenWolf connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openwolf plugins install @openwolf/twitch
```

Local checkout (when running from a git repo):

```bash
openwolf plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENWOLF_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openwolf", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENWOLF_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENWOLF_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
