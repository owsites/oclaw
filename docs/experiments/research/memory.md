...
read_when:
  - Designing workspace memory (~/.openwolf/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenWolf integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openwolf/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openwolf/workspace` as canonical human-readable memory.
...
```
~/.openwolf/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenWolf**, but keep a separable core library.

### Why integrate into OpenWolf?

- OpenWolf already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openwolf memory recall "…" --k 25 --since 30d`
  - `openwolf memory reflect --since 7d`
...
Pragmatic take for `~/.openwolf/workspace`:
