...
- Publish as `openwolf/plugin-sdk` (or export from core under `openwolf/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenWolfPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenWolfConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenWolfConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenWolfConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenWolfConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenWolfConfig): string;
  };
};
```
...
- Introduce `openwolf/plugin-sdk`.
- Add `api.runtime` to `OpenWolfPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openwolfRuntime: ">=2026.2.0"`).
