...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.openwolf/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.openwolf/node.json` (node id + pairing token).
...
Path: `~/.openwolf/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openwolf/exec-approvals.sock",
...
- Unix socket at `~/.openwolf/exec-approvals.sock` (0600).
