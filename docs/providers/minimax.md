---
summary: "Use MiniMax M2.1 in OpenWolf"
read_when:
  - You want MiniMax models in OpenWolf
...
```bash
openwolf plugins enable minimax-portal-auth  # skip if already loaded.
openwolf gateway restart  # restart if gateway is already running
openwolf onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/openwolf/openwolf/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openwolf.json`:
...
- Use `openwolf models list` and `openwolf models set minimax/MiniMax-M2.1` to switch.
...
- Running `openwolf configure` and selecting **MiniMax M2.1**, or
