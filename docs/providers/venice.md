---
summary: "Use Venice AI privacy-focused models in OpenWolf"
read_when:
  - You want privacy-focused inference in OpenWolf
...
```bash
openwolf onboard --auth-choice venice-api-key
```
...
```bash
openwolf chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, OpenWolf shows all available Venice models. Pick based on your needs:
...
```bash
openwolf models set venice/claude-opus-45
openwolf models set venice/llama-3.3-70b
```

List all available models:

```bash
openwolf models list | grep venice
```

## Configure via `openwolf configure`

1. Run `openwolf configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenWolf automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openwolf chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openwolf chat --model venice/claude-opus-45

# Use uncensored model
openwolf chat --model venice/venice-uncensored

# Use vision model with image
openwolf chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openwolf chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openwolf models list` to see currently available models. Some models may be temporarily offline.
