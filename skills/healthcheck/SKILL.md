---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenWolf deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenWolf cron scheduling for periodic checks, or version status checks on a machine running OpenWolf (laptop, workstation, Pi, VPS).
---

# OpenWolf Host Hardening

## Overview

Assess and harden the host running OpenWolf, then align it to a user-defined risk tolerance without breaking access. Use OpenWolf security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenWolf changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenWolf gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenWolf can access sensitive data.
...
### 2) Run OpenWolf security audits (read-only)

As part of the default read-only checks, run `openwolf security audit --deep`. Only offer alternatives if the user requests them:

1. `openwolf security audit` (faster, non-probing)
2. `openwolf security audit --json` (structured output)

Offer to apply OpenWolf safe defaults (numbered):

1. `openwolf security audit --fix`

Be explicit that `--fix` only tightens OpenWolf defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenWolf version/update status (read-only)

As part of the default read-only checks, run `openwolf update status`.
...
- Credential hygiene notes (location of OpenWolf creds, prefer disk encryption)
...
- OpenWolf security audit (re-run)
...
After OpenWolf install or first hardening pass, run at least one baseline audit and version check:

- `openwolf security audit`
- `openwolf security audit --deep`
- `openwolf update status`

Ongoing monitoring is recommended. Use the OpenWolf cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openwolf cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openwolf update status`
...
Before creating, `openwolf cron list` and match on exact `name`. If found, `openwolf cron edit <id> ...`.
If not found, `openwolf cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openwolf update status` (preferred for source checkouts and channels)
2. `npm view openwolf version` (published npm version)

## OpenWolf command accuracy

Use only supported commands and flags:

- `openwolf security audit [--deep] [--fix] [--json]`
- `openwolf status` / `openwolf status --deep`
- `openwolf health --json`
- `openwolf update status`
- `openwolf cron add|list|runs|run`

Do not invent CLI flags or imply OpenWolf enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenWolf compaction:
