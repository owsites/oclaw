...
metadata:
  { "openwolf": { "emoji": "🧵", "os": ["darwin", "linux"], "requires": { "bins": ["tmux"] } } }
---
...
```bash
SOCKET_DIR="${OPENWOLF_TMUX_SOCKET_DIR:-${WOLFBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/openwolf-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/openwolf.sock"
SESSION=openwolf-python
...
- Use `OPENWOLF_TMUX_SOCKET_DIR` (legacy `WOLFBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$OPENWOLF_TMUX_SOCKET_DIR/openwolf.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `OPENWOLF_TMUX_SOCKET_DIR`).
