...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openwolf/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openwolf");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openwolf/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENWOLF_CONTROL_UI_BASE_PATH__ = "/openwolf";
    const app = mountApp("/openwolf/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openwolf");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openwolf/sessions");
  });
