...
export async function handleWhatsAppStart(host: OpenWolfApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenWolfApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenWolfApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenWolfApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenWolfApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenWolfApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpenWolfApp): string | null {
...
function buildGatewayHttpHeaders(host: OpenWolfApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpenWolfApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenWolfApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenWolfApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenWolfApp) {
  const state = host.nostrProfileFormState;
