...
This document describes how the OpenWolf ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openwolf acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenWolf Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a OpenWolf Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `openwolf acp` over stdio.

Example config:

```bash
openwolf config set gateway.remote.url wss://gateway-host:18789
openwolf config set gateway.remote.token <token>
```

Example run:

```bash
openwolf acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
openwolf acp --session agent:main:main
openwolf acp --session agent:design:main
openwolf acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “OpenWolf ACP” to start a thread.

## Execution Model

- ACP client spawns `openwolf acp` and speaks ACP messages over stdio.
...
```bash
openwolf acp --session agent:main:main
openwolf acp --session-label "support inbox"
openwolf acp --reset-session
```
...
`openwolf acp` resolves the Gateway URL and auth from CLI flags or config:
