...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openwolf-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openwolf");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENWOLF_SKIP_BROWSER_CONTROL_SERVER: "1",
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-openwolf-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-openwolf-token": gwB.hookToken },
        ),
      ]);
