...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openwolfHome: process.env.OPENWOLF_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENWOLF_HOME", snapshot.openwolfHome);
  restoreKey("OPENWOLF_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENWOLF_HOME.
  delete process.env.OPENWOLF_HOME;
  process.env.OPENWOLF_STATE_DIR = path.join(base, ".openwolf");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openwolf-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openwolf", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
