...
import type { OpenWolfConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("openwolf-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("openwolf-bin-sherpa-");
      const modelDir = await makeTempDir("openwolf-sherpa-model-");
      ...
      const cfg: OpenWolfConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openwolf-bin-whispercpp-");
      const modelDir = await makeTempDir("openwolf-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openwolf-bin-gemini-");
      ...
      const cfg: OpenWolfConfig = { tools: { media: { image: {} } } };
