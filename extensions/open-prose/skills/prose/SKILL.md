...
metadata: { "openwolf": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenWolf `sessions_spawn`
- **File I/O** == OpenWolf `read`/`write`
- **Remote fetch** == OpenWolf `web_fetch` (or `exec` with curl when POST is required)
