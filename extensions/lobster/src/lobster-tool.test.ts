...
import type { OpenWolfPluginApi, OpenWolfPluginToolContext } from "../../../src/plugins/types.js";
...
function fakeApi(overrides: Partial<OpenWolfPluginApi> = {}): OpenWolfPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<OpenWolfPluginToolContext> = {}): OpenWolfPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-lobster-plugin-"));
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenWolfPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
