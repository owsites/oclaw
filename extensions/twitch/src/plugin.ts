/**
 * Twitch channel plugin for OpenWolf.
 *
...
import { buildChannelConfigSchema } from "openwolf/plugin-sdk";
import type {
 ...
 * for OpenWolf. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenWolfConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenWolfConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenWolfConfig): boolean => {
