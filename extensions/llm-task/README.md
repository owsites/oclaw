...
`openwolf.invoke --each`) without adding new OpenWolf code per workflow.
...
This extension depends on OpenWolf internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenWolf extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openwolf/extensions` as a standalone plugin directory.
