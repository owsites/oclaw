import type { PluginRuntime } from "openwolf/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENWOLF_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-nostr-"));
  process.env.OPENWOLF_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.OPENWOLF_STATE_DIR?.trim() || env.OPENWOLF_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".openwolf");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.OPENWOLF_STATE_DIR = previous;
    }
