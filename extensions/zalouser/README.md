...
OpenWolf extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
openwolf channels login --channel zalouser
...
```bash
openwolf message send --channel zalouser --target <threadId> --message "Hello from OpenWolf!"
```
...
```bash
openwolf channels login --channel zalouser              # Login via QR
openwolf channels login --channel zalouser --account work
openwolf channels status --probe
openwolf channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
openwolf directory self --channel zalouser
openwolf directory peers list --channel zalouser --query "name"
openwolf directory groups list --channel zalouser --query "work"
openwolf directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
openwolf message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
openwolf message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `openwolf channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
openwolf channels login --channel zalouser --account work
openwolf message send --channel zalouser --account work --target <id> --message "Hello"
