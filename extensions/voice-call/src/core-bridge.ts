...
function resolveOpenWolfRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENWOLF_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENWOLF_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenWolfRoot(), "dist", "extensionAPI.js");
