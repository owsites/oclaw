...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OpenWolfConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OpenWolfConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as OpenWolfConfig);
