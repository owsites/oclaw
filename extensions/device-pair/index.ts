...
import { approveDevicePairing, listDevicePairing } from "openwolf/plugin-sdk";
...
function resolveGatewayPort(cfg: OpenWolfPluginApi["config"]): number {
  const envRaw =
    process.env.OPENWOLF_GATEWAY_PORT?.trim() || process.env.WOLFBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: OpenWolfPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: OpenWolfPluginApi["config"]): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    process.env.OPENWOLF_GATEWAY_TOKEN?.trim() ||
    process.env.WOLFBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    process.env.OPENWOLF_GATEWAY_PASSWORD?.trim() ||
    process.env.WOLFBOT_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayUrl(api: OpenWolfPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenWolf iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenWolf iOS app:",
            "",
            "```",
            qrAscii,
            "```",
