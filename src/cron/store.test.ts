...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENWOLF_HOME for tilde expansion", () => {
    vi.stubEnv("OPENWOLF_HOME", "/srv/openwolf-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/openwolf-home", "cron", "jobs.json"));
  });
});
