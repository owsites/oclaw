...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENWOLF_GATEWAY_PORT).toBe("18789");
    expect(env.OPENWOLF_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENWOLF_SERVICE_MARKER).toBe("openwolf");
    expect(env.OPENWOLF_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENWOLF_SERVICE_VERSION).toBe("string");
    expect(env.OPENWOLF_SYSTEMD_UNIT).toBe("openwolf-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENWOLF_LAUNCHD_LABEL).toBe("ai.openwolf.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENWOLF_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENWOLF_SYSTEMD_UNIT).toBe("openwolf-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENWOLF_LAUNCHD_LABEL).toBe("ai.openwolf.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openwolf"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENWOLF_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openwolf-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENWOLF_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openwolf"));
  });

  it("uses OPENWOLF_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENWOLF_STATE_DIR: "/var/lib/openwolf" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openwolf"));
  });

  it("expands ~ in OPENWOLF_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENWOLF_STATE_DIR: "~/openwolf-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openwolf-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENWOLF_STATE_DIR: "C:\\State\\openwolf" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\openwolf");
  });
});
