...
describe("canvas host state dir defaults", () => {
  ...
  it("uses OPENWOLF_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("OpenWolf Canvas");
    } finally {
      await handler.close();
