  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/openwolf",
      },
    },
    session: { store: "/tmp/openwolf-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = {
        ...
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bopenwolf\\b"] },
        },
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "OpenWolf" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/openwolf" },
      session: { store: "/tmp/openwolf-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
