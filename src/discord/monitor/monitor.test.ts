...
describe("agent components", () => {
  const createCfg = (): OpenWolfConfig => ({}) as OpenWolfConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpenWolfConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockReset().mockReturnValue("/tmp/openwolf-sessions-test.json");
  });
