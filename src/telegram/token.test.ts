...
function withTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-telegram-token-"));
}

async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.OPENWOLF_STATE_DIR;
  const dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "openwolf-telegram-"));
...
describe("resolveTelegramToken", () => {
  ...
  it("uses tokenFile when configured", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile } } } as OpenWolfConfig;
    const res = resolveTelegramToken(cfg);
