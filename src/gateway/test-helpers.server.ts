...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENWOLF_STATE_DIR;
  previousConfigPath = process.env.OPENWOLF_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENWOLF_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENWOLF_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENWOLF_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.OPENWOLF_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.OPENWOLF_SKIP_CHANNELS;
  previousSkipProviders = process.env.OPENWOLF_SKIP_PROVIDERS;
  previousSkipCron = process.env.OPENWOLF_SKIP_CRON;
  previousMinimalGateway = process.env.OPENWOLF_TEST_MINIMAL_GATEWAY;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENWOLF_STATE_DIR = path.join(tempHome, ".openwolf");
  delete process.env.OPENWOLF_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENWOLF_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENWOLF_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENWOLF_SKIP_CANVAS_HOST = "1";
  process.env.OPENWOLF_SKIP_CHANNELS = "1";
  process.env.OPENWOLF_SKIP_PROVIDERS = "1";
  process.env.OPENWOLF_SKIP_CRON = "1";
  process.env.OPENWOLF_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENWOLF_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openwolf-test-no-bundled-extensions")
    : "openwolf-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "openwolf-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".openwolf-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPENWOLF_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENWOLF_CONFIG_PATH;
    } else {
      process.env.OPENWOLF_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENWOLF_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENWOLF_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENWOLF_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENWOLF_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENWOLF_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENWOLF_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.OPENWOLF_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.OPENWOLF_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.OPENWOLF_SKIP_CHANNELS;
    } else {
      process.env.OPENWOLF_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.OPENWOLF_SKIP_PROVIDERS;
    } else {
      process.env.OPENWOLF_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.OPENWOLF_SKIP_CRON;
    } else {
      process.env.OPENWOLF_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.OPENWOLF_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.OPENWOLF_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENWOLF_GATEWAY_TOKEN"]);
  const prev = process.env.OPENWOLF_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENWOLF_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.OPENWOLF_GATEWAY_PASSWORD;
