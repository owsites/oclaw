...
beforeAll(async () => {
  previousToken = process.env.OPENWOLF_GATEWAY_TOKEN;
  process.env.OPENWOLF_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.OPENWOLF_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "openwolf-ios", platform: "ios" },
  { clientId: "openwolf-android", platform: "android" },
