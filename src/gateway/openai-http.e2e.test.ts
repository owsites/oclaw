    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "openwolf", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-openwolf-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-openwolf-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "openwolf", messages: [{ role: "user", content: "hi" }] },
          {
            "x-openwolf-agent-id": "beta",
            "x-openwolf-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
