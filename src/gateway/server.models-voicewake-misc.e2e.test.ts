...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.OPENWOLF_STATE_DIR = path.join(homeDir, ".openwolf");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-home-"));
      const restoreHome = setTempHome(homeDir);

      const initial = await rpcReq<{ triggers: string[] }>(ws, "voicewake.get");
      expect(initial.ok).toBe(true);
      expect(initial.payload?.triggers).toEqual(["openwolf", "claude", "computer"]);
      ...
      const onDisk = JSON.parse(
        await fs.readFile(path.join(homeDir, ".openwolf", "settings", "voicewake.json"), "utf8"),
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    const envSnapshot = captureEnv(["OPENWOLF_CANVAS_HOST_PORT", "OPENWOLF_GATEWAY_TOKEN"]);
    try {
      ...
      process.env.OPENWOLF_CANVAS_HOST_PORT = String(canvasPort);
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    const configPath = process.env.OPENWOLF_CONFIG_PATH;
    if (!configPath) {
      throw new Error("Missing OPENWOLF_CONFIG_PATH");
    }
