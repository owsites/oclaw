...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.OPENWOLF_LIVE_TEST);
const CLI_LIVE = isTruthyEnvValue(process.env.OPENWOLF_LIVE_CLI_BACKEND);
const CLI_IMAGE = isTruthyEnvValue(process.env.OPENWOLF_LIVE_CLI_BACKEND_IMAGE_PROBE);
const CLI_RESUME = isTruthyEnvValue(process.env.OPENWOLF_LIVE_CLI_BACKEND_RESUME_PROBE);
...
function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("OPENWOLF_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    const previous = {
      configPath: process.env.OPENWOLF_CONFIG_PATH,
      token: process.env.OPENWOLF_GATEWAY_TOKEN,
      skipChannels: process.env.OPENWOLF_SKIP_CHANNELS,
      skipGmail: process.env.OPENWOLF_SKIP_GMAIL_WATCHER,
      skipCron: process.env.OPENWOLF_SKIP_CRON,
      skipCanvas: process.env.OPENWOLF_SKIP_CANVAS_HOST,
    ...
    const rawModel = process.env.OPENWOLF_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
    const parsed = parseModelRef(rawModel, "claude-cli");
    if (!parsed) {
      throw new Error(
        `OPENWOLF_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
      );
    }
    ...
    const cliCommand = process.env.OPENWOLF_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
    if (!cliCommand) {
      throw new Error(
        `OPENWOLF_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
      );
    }
    const baseCliArgs =
      parseJsonStringArray(
        "OPENWOLF_LIVE_CLI_BACKEND_ARGS",
        process.env.OPENWOLF_LIVE_CLI_BACKEND_ARGS,
      ) ?? providerDefaults?.args;
    if (!baseCliArgs || baseCliArgs.length === 0) {
      throw new Error(`OPENWOLF_LIVE_CLI_BACKEND_ARGS is required for provider "${providerId}".`);
    }
    const cliClearEnv =
      parseJsonStringArray(
        "OPENWOLF_LIVE_CLI_BACKEND_CLEAR_ENV",
        process.env.OPENWOLF_LIVE_CLI_BACKEND_CLEAR_ENV,
      ) ?? (providerId === "claude-cli" ? DEFAULT_CLEAR_ENV : []);
    const cliImageArg = process.env.OPENWOLF_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || undefined;
    const cliImageMode = parseImageMode(process.env.OPENWOLF_LIVE_CLI_BACKEND_IMAGE_MODE);

    if (cliImageMode && !cliImageArg) {
      throw new Error(
        "OPENWOLF_LIVE_CLI_BACKEND_IMAGE_MODE requires OPENWOLF_LIVE_CLI_BACKEND_IMAGE_ARG.",
      );
    }

    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-live-cli-"));
    const disableMcpConfig = process.env.OPENWOLF_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
    ...
    const tempConfigPath = path.join(tempDir, "openwolf.json");
    await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
    process.env.OPENWOLF_CONFIG_PATH = tempConfigPath;
