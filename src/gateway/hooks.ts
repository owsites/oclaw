...
export function resolveHooksConfig(cfg: OpenWolfConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenWolfConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openwolf-token"] === "string"
      ? req.headers["x-openwolf-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
