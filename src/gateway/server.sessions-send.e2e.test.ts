...
const { createOpenWolfTools } = await import("../agents/openwolf-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENWOLF_GATEWAY_PORT", "OPENWOLF_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENWOLF_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENWOLF_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createOpenWolfTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", { timeout: 60_000 }, async () => {
    ...
    if (!configPath) {
      throw new Error("OPENWOLF_CONFIG_PATH missing in gateway test environment");
    }
