...
import { createOpenWolfTools } from "../agents/openwolf-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-openwolf-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-openwolf-account-id")?.trim() || undefined;
  ...
  const allTools = createOpenWolfTools({
