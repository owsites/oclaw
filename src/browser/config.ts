import type { BrowserConfig, BrowserProfileConfig, OpenWolfConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENWOLF_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "openwolf" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OPENWOLF_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openwolf" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENWOLF_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENWOLF_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenWolf browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openwolf/openwolf CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenWolfConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENWOLF_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openwolf";
