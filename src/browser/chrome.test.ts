...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "openwolf-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateOpenWolfProfile(userDataDir, { color: DEFAULT_OPENWOLF_BROWSER_COLOR });
    ...
    expect(def.name).toBe(DEFAULT_OPENWOLF_BROWSER_PROFILE_NAME);
    expect(def.shortcut_name).toBe(DEFAULT_OPENWOLF_BROWSER_PROFILE_NAME);
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".openwolf-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateOpenWolfProfile(userDataDir, { color: "lobster-orange" });
    const localState = await readJson(path.join(userDataDir, "Local State"));
    const profile = localState.profile as Record<string, unknown>;
    const infoCache = profile.info_cache as Record<string, unknown>;
    const def = infoCache.Default as Record<string, unknown>;
  ...
  it("is idempotent when rerun on an existing profile", async () => {
    ...
    expect(profile.name).toBe(DEFAULT_OPENWOLF_BROWSER_PROFILE_NAME);
  });
});
...
describe("browser chrome helpers", () => {
  ...
  it("stopOpenWolfChrome no-ops when process is already killed", async () => {
    const proc = { killed: true, exitCode: null, kill: vi.fn() };
    await stopOpenWolfChrome(
      {
        proc,
        cdpPort: 12345,
      } as unknown as Parameters<typeof stopOpenWolfChrome>[0],
      10,
    );
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopOpenWolfChrome sends SIGTERM and returns once CDP is down", async () => {
