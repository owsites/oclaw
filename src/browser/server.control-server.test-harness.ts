...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/openwolf" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "openwolf",
        profiles: {
          openwolf: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenWolfChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.OPENWOLF_GATEWAY_PORT;
    process.env.OPENWOLF_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.OPENWOLF_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.OPENWOLF_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.OPENWOLF_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.OPENWOLF_GATEWAY_TOKEN;
    } else {
      process.env.OPENWOLF_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.OPENWOLF_GATEWAY_PASSWORD;
    } else {
      process.env.OPENWOLF_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
