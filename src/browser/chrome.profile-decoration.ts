...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openwolf-profile-decorated");
}
 ...
 */
export function decorateOpenWolfProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENWOLF_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENWOLF_BROWSER_COLOR).toUpperCase();
