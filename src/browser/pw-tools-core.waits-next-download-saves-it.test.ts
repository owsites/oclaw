...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredOpenWolfTmpDir: vi.fn(() => "/tmp/openwolf"),
}));
vi.mock("../infra/tmp-openwolf-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredOpenWolfTmpDir.mockReturnValue("/tmp/openwolf");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredOpenWolfTmpDir.mockReturnValue("/tmp/openwolf-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "openwolf-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredOpenWolfTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "openwolf-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "openwolf-preferred", "downloads")}${path.sep}`),
    );
  });
