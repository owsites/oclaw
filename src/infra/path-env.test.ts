...
let ensureOpenWolfCliOnPath: typeof import("./path-env.js").ensureOpenWolfCliOnPath;

describe("ensureOpenWolfCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENWOLF_PATH_BOOTSTRAPPED",
    "OPENWOLF_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenWolfCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openwolf exists", () => {
    const tmp = abs("/tmp/openwolf-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "openwolf");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPENWOLF_PATH_BOOTSTRAPPED;

    ensureOpenWolfCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENWOLF_PATH_BOOTSTRAPPED = "1";
    ensureOpenWolfCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/openwolf-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openwolf");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/openwolf-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openwolf");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "openwolf");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openwolf-path/case-linuxbrew");
