...
export function triggerOpenWolfRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENWOLF_SYSTEMD_UNIT,
        process.env.OPENWOLF_PROFILE,
      );
  ...
  const label =
    process.env.OPENWOLF_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENWOLF_PROFILE);
