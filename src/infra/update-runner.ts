...
const DEFAULT_PACKAGE_NAME = "openwolf";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openwolf.mjs");
    ...
    const doctorStep = await runStep(
      step("openwolf doctor", doctorArgv, gitRoot, { OPENWOLF_UPDATE_IN_PROGRESS: "1" }),
    );
