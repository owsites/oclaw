...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__openwolf_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "openwolf-root");
...
describe("resolveOpenWolfPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveOpenWolfPackageRootSync } = await import("./openwolf-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "openwolf");
    const pkgRoot = path.join(project, "node_modules", "openwolf");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openwolf" }));

    expect(resolveOpenWolfPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "openwolf");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "openwolf.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "openwolf" }));

    expect(resolveOpenWolfPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOpenWolfPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-openwolf package roots", async () => {
    const { resolveOpenWolfPackageRootSync } = await import("./openwolf-root.js");

    const pkgRoot = fx("not-openwolf");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-openwolf" }));

    expect(resolveOpenWolfPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveOpenWolfPackageRoot } = await import("./openwolf-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openwolf" }));

    await expect(resolveOpenWolfPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
