...
vi.mock("./openwolf-root.js", () => ({
  resolveOpenWolfPackageRoot: vi.fn(async () => null),
  resolveOpenWolfPackageRootSync: vi.fn(() => null),
}));
...
describe("control UI assets helpers (fs-mocked)", () => {
  ...
  it("uses resolveOpenWolfPackageRoot when available", async () => {
    const openwolfRoot = await import("./openwolf-root.js");
    const { resolveControlUiDistIndexPath } = await import("./control-ui-assets.js");

    const pkgRoot = abs("fixtures/openwolf");
    (
      openwolfRoot.resolveOpenWolfPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openwolf"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openwolf" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openwolf.mjs"))).resolves.toBe(
  ...
  it("returns null when fallback package name does not match", async () => {
    ...
    const root = abs("fixtures/not-openwolf");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    ...
    const pkgRoot = abs("fixtures/openwolf-bundle");
    (
      openwolfRoot.resolveOpenWolfPackageRootSync as unknown as ReturnType<typeof vi.fn>
