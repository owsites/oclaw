...
describe("delivery-queue", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-dq-test-"));
  });
...
describe("resolveOutboundSessionRoute", () => {
  const baseConfig = {} as OpenWolfConfig;
  ...
  it("treats Telegram usernames as DMs when unresolved", async () => {
    const cfg = { session: { dmScope: "per-channel-peer" } } as OpenWolfConfig;
    const route = await resolveOutboundSessionRoute({
      cfg,
      channel: "telegram",
      agentId: "main",
