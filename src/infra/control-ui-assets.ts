...
import { resolveOpenWolfPackageRoot, resolveOpenWolfPackageRootSync } from "./openwolf-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOpenWolfPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "openwolf" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOpenWolfPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // openwolf.mjs or dist/<bundle>.js
