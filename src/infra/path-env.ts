...
function candidateBinDirs(opts: EnsureOpenWolfPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `openwolf` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openwolf");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENWOLF_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "openwolf"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openwolf` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenWolfCliOnPath(opts: EnsureOpenWolfPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENWOLF_PATH_BOOTSTRAPPED)) {
    return;
  }
