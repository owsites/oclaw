...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENWOLF_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "OpenWolf";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(OpenWolf\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.OPENWOLF_MDNS_HOSTNAME?.trim() ||
    process.env.WOLFBOT_MDNS_HOSTNAME?.trim() ||
    "openwolf";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
