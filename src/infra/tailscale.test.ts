...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.OPENWOLF_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.OPENWOLF_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.OPENWOLF_TEST_TAILSCALE_BINARY;
    } else {
      process.env.OPENWOLF_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
