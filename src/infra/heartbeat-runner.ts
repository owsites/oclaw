...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OpenWolfConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: OpenWolfConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: OpenWolfConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: OpenWolfConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: OpenWolfConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: OpenWolfConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OpenWolfConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
