...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENWOLF_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openwolf-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openwolf";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openwolf");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openwolf-gw");
  ...
  records.push(`_openwolf-gw._tcp IN PTR ${instanceLabel}._openwolf-gw._tcp`);
  records.push(`${instanceLabel}._openwolf-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openwolf-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openwolf-content-hash: ${contentHash}\n${contentBody}`;
}
