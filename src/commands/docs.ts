...
const SEARCH_TOOL = "https://docs.openwolf.ai/mcp.SearchOpenWolf";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openwolf.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openwolf docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openwolf.ai/");
      runtime.log(`Search: ${formatCliCommand('openwolf docs "your query"')}`);
    }
    return;
  }
