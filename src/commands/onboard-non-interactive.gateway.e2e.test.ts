...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.OPENWOLF_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("openwolf-onboard-");
  ...
  afterAll(async () => {
    ...
    process.env.OPENWOLF_GATEWAY_PASSWORD = prev.password;
  });

  it("writes gateway token auth into config and gateway enforces it", async () => {
    await withStateDir("state-noninteractive-", async (stateDir) => {
      const token = "tok_test_123";
      const workspace = path.join(stateDir, "openwolf");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.OPENWOLF_STATE_DIR = stateDir;
      process.env.OPENWOLF_CONFIG_PATH = path.join(stateDir, "openwolf.json");
