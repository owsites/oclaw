...
export function listAgentEntries(cfg: OpenWolfConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenWolfConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenWolfConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenWolfConfig): AgentSummary[] {
