...
export async function updateConfig(
  mutator: (cfg: OpenWolfConfig) => OpenWolfConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenWolfConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenWolfConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openwolf agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
