...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenWolfAgentDir: vi.fn().mockReturnValue("/tmp/openwolf-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openwolf-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openwolf-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenWolfAgentDir: mocks.resolveOpenWolfAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenWolfAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openwolf-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/openwolf-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/openwolf-agent-custom");
