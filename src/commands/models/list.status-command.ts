...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenWolfAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openwolf models auth setup-token")}\` or \`${formatCliCommand("openwolf configure")}\`.`
          : `Run \`${formatCliCommand("openwolf configure")}\` or set an API key env var.`;
