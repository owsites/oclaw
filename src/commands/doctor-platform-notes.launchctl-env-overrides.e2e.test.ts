...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENWOLF_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENWOLF_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENWOLF_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENWOLF_GATEWAY_PASSWORD");
  });
