...
export const resolveOpenWolfPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenWolfPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOpenWolfPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENWOLF_STATE_DIR;
  originalUpdateInProgress = process.env.OPENWOLF_UPDATE_IN_PROGRESS;
  process.env.OPENWOLF_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENWOLF_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENWOLF_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENWOLF_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
