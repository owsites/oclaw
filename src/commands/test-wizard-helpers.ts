...
export function requireOpenWolfAgentDir(): string {
  const agentDir = process.env.OPENWOLF_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENWOLF_AGENT_DIR not set");
  }
  return agentDir;
}
