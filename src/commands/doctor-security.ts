...
import type { OpenWolfConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenWolfConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openwolf security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openwolf configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openwolf config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openwolf doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openwolf config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('openwolf config set session.dmScope "per-channel-peer"') +
