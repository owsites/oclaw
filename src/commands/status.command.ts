...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenWolf status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openwolf security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openwolf security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openwolf.ai/faq");
  runtime.log("Troubleshooting: https://docs.openwolf.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openwolf status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openwolf logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openwolf status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openwolf gateway probe")}`);
  }
}
