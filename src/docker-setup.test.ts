...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "openwolf-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENWOLF_GATEWAY_TOKEN: "test-token",
    OPENWOLF_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENWOLF_WORKSPACE_DIR: join(sandbox.rootDir, "openwolf"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        OPENWOLF_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        OPENWOLF_EXTRA_MOUNTS: undefined,
        OPENWOLF_HOME_VOLUME: "openwolf-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENWOLF_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENWOLF_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENWOLF_HOME_VOLUME=openwolf-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("openwolf-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("openwolf-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENWOLF_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
