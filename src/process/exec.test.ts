...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["OPENWOLF_BASE_ENV"]);
    process.env.OPENWOLF_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.OPENWOLF_BASE_ENV ?? "") + "|" + (process.env.OPENWOLF_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { OPENWOLF_TEST_ENV: "ok" },
        },
      );
