...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { OPENWOLF_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { OPENWOLF_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports OPENWOLF_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { OPENWOLF_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { OPENWOLF_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { OPENWOLF_SMOKE: "nope" })).toBe("qr");
  });
});
