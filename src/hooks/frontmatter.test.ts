...
`;
    ...
    expect(parsed.openwolf.emoji).toBe("💾");
    expect(parsed.openwolf.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openwolf.emoji).toBe("📝");
    expect(parsed.openwolf.events).toEqual(["command"]);
    expect(parsed.openwolf.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openwolf.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openwolf": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openwolf": {"events": ["test"]}}');
  });
...
describe("resolveOpenWolfMetadata", () => {
  it("extracts openwolf metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenWolfMetadata(frontmatter);
  ...
  it("returns undefined when openwolf key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openwolf: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenWolf" },
            { id: "npm", kind: "npm", package: "@openwolf/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openwolf/hook");
  });
...
`;
    ...
    const openwolf = resolveOpenWolfMetadata(frontmatter);
    expect(openwolf).toBeDefined();
    expect(openwolf?.emoji).toBe("💾");
    expect(openwolf?.events).toEqual(["command:new"]);
    expect(openwolf?.requires?.config).toEqual(["workspace.dir"]);
    expect(openwolf?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openwolf = resolveOpenWolfMetadata(frontmatter);
    expect(openwolf?.emoji).toBe("disk");
    expect(openwolf?.events).toEqual(["command:new"]);
  });
});
