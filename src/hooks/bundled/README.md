...
This directory contains hooks that ship with OpenWolf. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.openwolf/openwolf.json`:
...
2. Restart gateway: `pkill -9 -f 'openwolf.*gateway' && pnpm openwolf gateway`
3. Enable the hook: `openwolf hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openwolf.ai/automation/hooks
