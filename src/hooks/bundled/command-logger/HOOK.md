...
homepage: https://docs.openwolf.ai/automation/hooks#command-logger
metadata:
  {
    "openwolf":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openwolf/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openwolf/logs/commands.log ~/.openwolf/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openwolf`:
   ```
   /home/username/.openwolf/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
