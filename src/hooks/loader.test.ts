...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.OPENWOLF_BUNDLED_HOOKS_DIR;
    process.env.OPENWOLF_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.OPENWOLF_BUNDLED_HOOKS_DIR;
    } else {
      process.env.OPENWOLF_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
