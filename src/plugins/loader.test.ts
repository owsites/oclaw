...
import { loadOpenWolfPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `openwolf-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENWOLF_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openwolf.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.OPENWOLF_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadOpenWolfPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENWOLF_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenWolfPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOpenWolfPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpenWolfPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
