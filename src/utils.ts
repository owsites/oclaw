...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openwolf");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENWOLF_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENWOLF_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENWOLF_STATE_DIR.
