...
function resolveGatewayPort(cfg: OpenWolfConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: OpenWolfConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.OPENWOLF_GATEWAY_TOKEN?.trim() ||
    env.WOLFBOT_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.OPENWOLF_GATEWAY_PASSWORD?.trim() ||
    env.WOLFBOT_GATEWAY_PASSWORD?.trim() ||
