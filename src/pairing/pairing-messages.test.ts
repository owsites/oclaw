...
describe("buildPairingReply", () => {
  ...
  for (const testCase of cases) {
    it(`formats pairing reply for ${testCase.channel}`, () => {
      ...
      // CLI commands should respect OPENWOLF_PROFILE when set (most tests run with isolated profile)
      const commandRe = new RegExp(
        `(?:openwolf|openwolf) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
      );
