...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openwolf-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openwolf when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openwolf-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENWOLF_HOME over HOME", () => {
    vi.stubEnv("OPENWOLF_HOME", "/srv/openwolf-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openwolf-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENWOLF_HOME prefix when OPENWOLF_HOME is set", () => {
    vi.stubEnv("OPENWOLF_HOME", "/srv/openwolf-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/openwolf-home")}/.openwolf/openwolf.json`)).toBe(
      "$OPENWOLF_HOME/.openwolf/openwolf.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENWOLF_HOME replacement when OPENWOLF_HOME is set", () => {
    vi.stubEnv("OPENWOLF_HOME", "/srv/openwolf-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/openwolf-home")}/.openwolf/openwolf.json`),
    ).toBe("config: $OPENWOLF_HOME/.openwolf/openwolf.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openwolf")).toBe(path.resolve(os.homedir(), "openwolf"));
  });
  ...
  it("prefers OPENWOLF_HOME for tilde expansion", () => {
    vi.stubEnv("OPENWOLF_HOME", "/srv/openwolf-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openwolf")).toBe(path.resolve("/srv/openwolf-home", "openwolf"));
