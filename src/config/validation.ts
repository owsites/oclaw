...
import type { OpenWolfConfig, ConfigValidationIssue } from "./types.js";
...
import { OpenWolfSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: OpenWolfConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenWolfConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenWolfSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenWolfConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenWolfConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenWolfConfig,
  };
}
