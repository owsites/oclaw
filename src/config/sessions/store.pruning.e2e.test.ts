...
// Keep integration tests deterministic: never read a real openwolf.json.
vi.mock("../config.js", () => ({
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-pruning-integ-"));
  });
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.OPENWOLF_SESSION_CACHE_TTL_MS;
    process.env.OPENWOLF_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.OPENWOLF_SESSION_CACHE_TTL_MS;
    } else {
      process.env.OPENWOLF_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
