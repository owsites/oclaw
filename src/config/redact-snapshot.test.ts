...
import type { ConfigFileSnapshot } from "./types.openwolf.js";
import {
  REDACTED_SENTINEL,
...
function makeSnapshot(config: Record<string, unknown>, raw?: string): ConfigFileSnapshot {
  return {
    path: "/home/user/.openwolf/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/openwolf/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/openwolf/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/openwolf/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/openwolf/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OpenWolfSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OpenWolfConfig";
    const hints = mapSensitivePaths(OpenWolfSchema, "", {});
