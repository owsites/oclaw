...
import type { OpenWolfConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareOpenWolfVersions } from "./version.js";
...
function coerceConfig(value: unknown): OpenWolfConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenWolfConfig;
}
...
function stampConfigVersion(cfg: OpenWolfConfig): OpenWolfConfig {
...
function warnIfConfigFromFuture(cfg: OpenWolfConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenWolfVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenWolf (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenWolfConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenWolfConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenWolfConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenWolfConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenWolfConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENWOLF_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENWOLF_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENWOLF_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENWOLF_WATCH_SESSION === "string" &&
        deps.env.OPENWOLF_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENWOLF_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENWOLF_WATCH_COMMAND === "string" &&
        deps.env.OPENWOLF_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENWOLF_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENWOLF_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENWOLF_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENWOLF_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
