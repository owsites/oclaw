...
const ETC_OPENWOLF_DIR = path.join(ROOT_DIR, "etc", "openwolf");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "openwolf.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpenWolfPath(...parts: string[]) {
  return path.join(ETC_OPENWOLF_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpenWolfPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "openwolf.json"))).toEqual({
      shared: true,
    });
  });
});
