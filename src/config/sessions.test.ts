...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-sessions-suite-"));
  });
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-openwolf",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-openwolf#general");
  });
  ...
  it("derives session transcripts dir from OPENWOLF_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENWOLF_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.OPENWOLF_STATE_DIR = "/custom/state";
    try {
