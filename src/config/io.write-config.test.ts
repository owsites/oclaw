...
describe("config io write", () => {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".openwolf", "openwolf.json");
  ...
  it("persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withTempHome("openwolf-config-io-", async (home) => {
  ...
  it("keeps env refs in arrays when appending entries", async () => {
    await withTempHome("openwolf-config-io-", async (home) => {
      const configPath = path.join(home, ".openwolf", "openwolf.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        JSON.stringify(
  ...
  it("appends config write audit JSONL entries with forensic metadata", async () => {
    await withTempHome("openwolf-config-io-", async (home) => {
      const configPath = path.join(home, ".openwolf", "openwolf.json");
      const auditPath = path.join(home, ".openwolf", "logs", "config-audit.jsonl");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        JSON.stringify({ gateway: { port: 18789 } }, null, 2),
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withTempHome("openwolf-config-io-", async (home) => {
      ...
      const io = createConfigIO({
        env: {
          OPENWOLF_WATCH_MODE: "1",
          OPENWOLF_WATCH_SESSION: "watch-session-1",
          OPENWOLF_WATCH_COMMAND: "gateway --force",
