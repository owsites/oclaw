...
import type { OpenWolfConfig } from "../config/types.js";
import type { ConsoleStyle } from "./console.js";
import { resolvePreferredOpenWolfTmpDir } from "../infra/tmp-openwolf-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredOpenWolfTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "openwolf.log"); // legacy single-file path

const LOG_PREFIX = "openwolf";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OpenWolfConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => OpenWolfConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.OPENWOLF_TEST_FILE_LOG !== "1" ? "silent" : "info";
