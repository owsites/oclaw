...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openwolf.sandbox=1");
  args.push("--label", `openwolf.sessionKey=${params.scopeKey}`);
  args.push("--label", `openwolf.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openwolf.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "openwolf.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openwolf sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openwolf sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openwolf sandbox recreate --all");
}
