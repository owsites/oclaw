...
function resolveExecConfig(params: { cfg?: OpenWolfConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: OpenWolfConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createOpenWolfCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createOpenWolfReadTool(freshReadTool);
