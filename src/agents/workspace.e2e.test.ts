...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENWOLF_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENWOLF_HOME: "/srv/openwolf-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openwolf-home"), ".openwolf", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".openwolf", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("openwolf-workspace-");
