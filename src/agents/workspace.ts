...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENWOLF_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".openwolf", `workspace-${profile}`);
  }
  return path.join(home, ".openwolf", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".openwolf";
