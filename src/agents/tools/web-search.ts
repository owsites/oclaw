...
type WebSearchConfig = NonNullable<OpenWolfConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenWolfConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("openwolf configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.openwolf.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://openwolf.ai",
      "X-Title": "OpenWolf Web Search",
    },
