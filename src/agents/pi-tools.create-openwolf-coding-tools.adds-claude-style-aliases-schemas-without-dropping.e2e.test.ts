...
import { __testing, createOpenWolfCodingTools } from "./pi-tools.js";
...
describe("createOpenWolfCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenWolfTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenWolfCodingTools({ messageProvider: "discord" });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-depth-policy-"));
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openwolf-outside.txt");
