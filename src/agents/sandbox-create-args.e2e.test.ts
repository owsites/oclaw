...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        ...
        "openwolf.sandbox=1",
        "--label",
        "openwolf.sessionKey=main",
        "--label",
        "openwolf.createdAtMs=1700000000000",
        "--label",
        "openwolf.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
  ...
  it("throws on dangerous bind mounts (parent path)", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "openwolf-sbx-dangerous-parent",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
      }),
