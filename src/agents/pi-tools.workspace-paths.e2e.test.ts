...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openwolf-ws-", async (workspaceDir) => {
      await withTempDir("openwolf-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenWolfCodingTools({ workspaceDir });
    ...
    await withTempDir("openwolf-ws-", async (workspaceDir) => {
      await withTempDir("openwolf-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello openwolf");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openwolf-ws-", async (workspaceDir) => {
      await withTempDir("openwolf-override-", async (overrideDir) => {
        const tools = createOpenWolfCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openwolf-sandbox-", async (sandboxDir) => {
      await withTempDir("openwolf-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createOpenWolfCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
