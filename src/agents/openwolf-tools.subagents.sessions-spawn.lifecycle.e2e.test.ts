...
} from "./openwolf-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateOpenWolfTools = (typeof import("./openwolf-tools.js"))["createOpenWolfTools"];
type CreateOpenWolfToolsOpts = Parameters<CreateOpenWolfTools>[0];

async function getSessionsSpawnTool(opts: CreateOpenWolfToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createOpenWolfTools } = await import("./openwolf-tools.js");
  const tool = createOpenWolfTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("openwolf-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
