...
let createOpenWolfTools: (typeof import("./openwolf-tools.js"))["createOpenWolfTools"];
...
describe("openwolf-tools: subagents steer failure", () => {
  beforeEach(async () => {
    vi.resetModules();
    ({ createOpenWolfTools } = await import("./openwolf-tools.js"));
    ...
    const storePath = path.join(
      os.tmpdir(),
      `openwolf-subagents-steer-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
    );
    setSubagentsConfigOverride({
      session: {
        mainKey: "main",
