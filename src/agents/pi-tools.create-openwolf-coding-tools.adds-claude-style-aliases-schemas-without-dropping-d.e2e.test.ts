...
describe("createOpenWolfCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from openwolf read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpenWolfCodingTools({ sandbox });
