...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): OpenWolfConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-workspace-"));
    const now = Date.now();
