...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const envSnapshot = captureEnv(["OPENWOLF_STATE_DIR", "OPENWOLF_PROFILE"]);
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-test-"));
    ...
    try {
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: openwolf --profile isolated doctor --non-interactive",
      );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/openwolf" } }\n}\n';
