...
const originalOpenWolfStateDir = process.env.OPENWOLF_STATE_DIR;

afterEach(() => {
  if (originalOpenWolfStateDir === undefined) {
    delete process.env.OPENWOLF_STATE_DIR;
  } else {
    process.env.OPENWOLF_STATE_DIR = originalOpenWolfStateDir;
  }
});
