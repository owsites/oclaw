...
function listAgents(cfg: OpenWolfConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OpenWolfConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenWolfConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OpenWolfConfig, EvaluatedBindingsCache>();
