...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { OPENWOLF_STATE_DIR: process.env.OPENWOLF_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.OPENWOLF_STATE_DIR === undefined) {
      delete process.env.OPENWOLF_STATE_DIR;
    } else {
      process.env.OPENWOLF_STATE_DIR = snapshot.OPENWOLF_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "openwolf-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "openwolf-memory-flush-"));
  });
