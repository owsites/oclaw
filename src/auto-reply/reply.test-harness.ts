...
type HomeEnvSnapshot = {
  ...
  OPENWOLF_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    OPENWOLF_AGENT_DIR: process.env.OPENWOLF_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.OPENWOLF_AGENT_DIR = path.join(home, ".openwolf", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".openwolf", "agent");
