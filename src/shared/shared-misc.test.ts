...
describe("shared/frontmatter", () => {
  ...
  test("resolveOpenWolfManifestBlock parses JSON5 metadata and picks openwolf block", () => {
    const frontmatter = {
      metadata: "{ openwolf: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOpenWolfManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOpenWolfManifestBlock returns undefined for invalid input", () => {
    expect(resolveOpenWolfManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOpenWolfManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOpenWolfManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
