...
const OPENWOLF_REPO_URL = "https://github.com/openwolf/openwolf.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "openwolf";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openwolf@")) {
    return trimmed.slice("openwolf@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.OPENWOLF_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOpenWolfPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENWOLF_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENWOLF_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENWOLF_GIT_DIR to an empty folder or an openwolf checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENWOLF_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "openwolf.mjs");
