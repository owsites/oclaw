...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    OPENWOLF_CONFIG_PATH: process.env.OPENWOLF_CONFIG_PATH,
    OPENWOLF_GATEWAY_PORT: process.env.OPENWOLF_GATEWAY_PORT,
    OPENWOLF_PROFILE: process.env.OPENWOLF_PROFILE,
  };

  beforeEach(() => {
    process.env.OPENWOLF_STATE_DIR = "/tmp/openwolf-cli-state";
    process.env.OPENWOLF_CONFIG_PATH = "/tmp/openwolf-cli-state/openwolf.json";
    delete process.env.OPENWOLF_GATEWAY_PORT;
    delete process.env.OPENWOLF_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.OPENWOLF_STATE_DIR !== undefined) {
      process.env.OPENWOLF_STATE_DIR = originalEnv.OPENWOLF_STATE_DIR;
    } else {
      delete process.env.OPENWOLF_STATE_DIR;
    }

    if (originalEnv.OPENWOLF_CONFIG_PATH !== undefined) {
      process.env.OPENWOLF_CONFIG_PATH = originalEnv.OPENWOLF_CONFIG_PATH;
    } else {
      delete process.env.OPENWOLF_CONFIG_PATH;
    }

    if (originalEnv.OPENWOLF_GATEWAY_PORT !== undefined) {
      process.env.OPENWOLF_GATEWAY_PORT = originalEnv.OPENWOLF_GATEWAY_PORT;
    } else {
      delete process.env.OPENWOLF_GATEWAY_PORT;
    }

    if (originalEnv.OPENWOLF_PROFILE !== undefined) {
      process.env.OPENWOLF_PROFILE = originalEnv.OPENWOLF_PROFILE;
    } else {
      delete process.env.OPENWOLF_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        OPENWOLF_PROFILE: "dev",
        OPENWOLF_STATE_DIR: "/tmp/openwolf-daemon-state",
        OPENWOLF_CONFIG_PATH: "/tmp/openwolf-daemon-state/openwolf.json",
        OPENWOLF_GATEWAY_PORT: "19001",
      },
