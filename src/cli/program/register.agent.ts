...
${formatHelpExamples([
  ['openwolf agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openwolf agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openwolf agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openwolf agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openwolf agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openwolf agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openwolf.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openwolf.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openwolf agents set-identity --agent main --name "OpenWolf" --emoji "🦞"', "Set name + emoji."],
  ["openwolf agents set-identity --agent main --avatar avatars/openwolf.png", "Set avatar path."],
  [
    "openwolf agents set-identity --workspace ~/.openwolf/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openwolf agents set-identity --identity-file ~/.openwolf/workspace/IDENTITY.md --agent main",
