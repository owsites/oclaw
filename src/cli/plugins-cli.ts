...
function applySlotSelectionForPlugin(
  ...
  pluginId: string,
): { config: OpenWolfConfig; warnings: string[] } {
...
function enablePluginInConfig(config: OpenWolfConfig, pluginId: string): OpenWolfConfig {
  return {
    ...config,
    plugins: {
      ...config.plugins,
...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage OpenWolf plugins/extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.openwolf.ai/cli/plugins")}\n`,
    );
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          let next: OpenWolfConfig = enablePluginInConfig(
            {
              ...cfg,
              plugins: {
                ...cfg.plugins,
  ...
  plugins
    ...
    .action(() => {
      ...
      const docs = formatDocsLink("/plugin", "docs.openwolf.ai/plugin");
      lines.push("");
