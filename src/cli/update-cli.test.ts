...
const { resolveOpenWolfPackageRoot } = await import("../infra/openwolf-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openwolf-update-tests-"));
  });
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("openwolf-update");
    setTty(false);
    readPackageVersion.mockResolvedValue("2.0.0");

    vi.mocked(resolveOpenWolfPackageRoot).mockResolvedValue(tempDir);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveOpenWolfPackageRoot).mockReset();
    ...
    vi.mocked(resolveOpenWolfPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("openwolf");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OpenWolf update status");
  });
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("openwolf-update-wizard");
    const envSnapshot = captureEnv(["OPENWOLF_GIT_DIR"]);
    try {
      setTty(true);
      process.env.OPENWOLF_GIT_DIR = tempDir;
