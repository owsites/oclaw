...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openwolf.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenWolf hooks")
    ...
    .option("--hook-url <url>", "OpenWolf hook URL")
    .option("--hook-token <token>", "OpenWolf hook token")
