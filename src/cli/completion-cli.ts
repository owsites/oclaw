...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# OpenWolf Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(openwolf completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# OpenWolf Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(openwolf completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $OPENWOLF_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "openwolf") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c openwolf -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'openwolf gateway' and not 'openwolf other gateway', we need to check the sequence.
