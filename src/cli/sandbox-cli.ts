...
const SANDBOX_EXAMPLES = {
  main: [
    ["openwolf sandbox list", "List all sandbox containers."],
    ["openwolf sandbox list --browser", "List only browser containers."],
    ["openwolf sandbox recreate --all", "Recreate all containers."],
    ["openwolf sandbox recreate --session main", "Recreate a specific session."],
    ["openwolf sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openwolf sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openwolf sandbox list", "List all sandbox containers."],
    ["openwolf sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["openwolf sandbox recreate --all", "Recreate all containers."],
    ["openwolf sandbox recreate --session main", "Recreate a specific session."],
    ["openwolf sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openwolf sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openwolf sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openwolf sandbox explain", "Show effective sandbox config."],
    ["openwolf sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openwolf sandbox explain --agent work", "Explain an agent sandbox."],
    ["openwolf sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openwolf.ai/cli/sandbox")}\n`,
    )
    .action(() => {
