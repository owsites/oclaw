...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openwolf-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENWOLF_NO_RESPAWN: "1",
      OPENWOLF_STATE_DIR: stateDir,
      OPENWOLF_SKIP_CHANNELS: "1",
      OPENWOLF_SKIP_GMAIL_WATCHER: "1",
      OPENWOLF_SKIP_CRON: "1",
      OPENWOLF_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENWOLF_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openwolf-entry-bootstrap.mjs");
