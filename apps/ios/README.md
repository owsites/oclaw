...
This is an **alpha** iOS app that connects to an OpenWolf Gateway as a `role: node`.
...
3. On iOS: OpenWolf → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project OpenWolf.xcodeproj -scheme OpenWolf -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/OpenWolfKit` contains the shared transport/types used by the iOS app.
